...
"La ilusión domina la realidad…"`)
  ...
  try {
    const apiUrl =
      `https://api.delirius.online/download/tiktok?url=${encodeURIComponent(url)}`
...
"Todo ocurre según mi voluntad…"`
    ...
    const authorHandle =
      (author.username || author.unique_id || 'user')
        .replace(/^@/, '')
...
"La ilusión domina la realidad…"`
    ...
    if (isMp3) {
      ...
      await msg.reply(
        '❖ La descarga en MP3 no está disponible para este proveedor; se envía la versión de video.'
      )
